...
interface HealthConditionsTabProps {
  initialConditions: Array<{ id: string; name: string }>;
...
export default function HealthConditionsTab({
  ...
  // Spellcheck and suggest corrections as user types
  useEffect(() => {
    ...
    // Find matches that include the typed string (prefix matching)
    const prefixMatches = commonConditions.filter(
      (condition) =>
        condition.toLowerCase().includes(newCondition.toLowerCase()) &&
        !conditions.some((c) => c.toLowerCase() === condition.toLowerCase())
    );

    // Also find potential spelling mistakes using Levenshtein distance
    const spellingMatches = commonConditions.filter((condition) => {
      // Don't suggest exact matches, conditions the user already has, or ones found by prefix
      if (
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        {/* Add condition input with spellcheck */}
        <div className="relative">
          <div className="flex gap-2 mb-2">
            <input
              ...
              onFocus={() => newCondition.length >= 3 && setShowSuggestions(true)}
